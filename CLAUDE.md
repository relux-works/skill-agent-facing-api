...
Separate Go module (`example/go.mod`) with a `taskdemo` CLI that wires up a `Task` domain type against the library. Shows how to register fields, presets, operations (`get`, `list`, `count`, `summary`) with metadata, mutations (`create`, `update`, `delete`) with mutation metadata, and use `cobraext.AddCommands`.
